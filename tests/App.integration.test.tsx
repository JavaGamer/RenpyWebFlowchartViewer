...
describe("App – upload → parse → render integration", () => {
  ...
  it("rejects oversized .rpy files with a clear message", async () => {
    ...
    await waitFor(() => {
      ...
      expect(view.getByText(/smaller than 2 MiB \(about 2 MB\)/i))
        .toBeInTheDocument();
    });
  });
  ...
  it("shows an empty-graph warning when the .rpy file has no labels or menus", async () => {
    ...
    await waitFor(() => {
      expect(
        view.getByText(
          /No labels or menus were found\. Make sure the folder contains/i,
        ),
      ).toBeInTheDocument();
  ...
  it("allows uploading a different folder after a successful parse", async () => {
    ...
    const resetBtn = view.getByRole("button", {
      name: /Upload a different folder/i,
    });
  ...
  it("shows a file-read error message when FileReader fails", async () => {
    ...
    try {
      ...
      await waitFor(() => {
        expect(view.getByText(/Could not read "broken\.rpy"/i))
          .toBeInTheDocument();
      });
    } finally {
  ...
  it("shows an actionable parse error message when the parser throws", async () => {
    ...
    await waitFor(() => {
      expect(
        view.getByText(
          /Failed to parse Ren'Py scripts.*Unexpected token at line 3/is,
        ),
      ).toBeInTheDocument();
      expect(
        view.getByText(/Ensure your \.rpy files contain valid Ren'Py syntax/i),
      ).toBeInTheDocument();
      expect(view.getByRole("button", { name: /Try again/i }))
        .toBeInTheDocument();
      expect(view.getByRole("button", { name: /Start over/i }))
        .toBeInTheDocument();
    });
  });
  ...
  it("persists parser variant and custom rules across remounts", async () => {
    ...
    const variantSelect = firstView.getByRole("combobox", {
      name: /Parser variant/i,
    });
    await user.selectOptions(variantSelect, "st");
    await user.click(
      firstView.getByRole("button", { name: /Add custom rule/i }),
    );
    const actionInput = firstView.getByRole("textbox", {
      name: /Custom rule action 1/i,
    });
    await user.type(actionInput, "Warp");
    const actionTypeSelect = firstView.getByRole("combobox", {
      name: /Custom rule action type 1/i,
    });
    ...
    expect(secondView.getByRole("combobox", { name: /Parser variant/i }))
      .toHaveValue("st");
    expect(secondView.getByRole("textbox", { name: /Custom rule action 1/i }))
      .toHaveValue("Warp");
    expect(
      secondView.getByRole("combobox", { name: /Custom rule action type 1/i }),
    ).toHaveValue("call");
  });
  ...
  it("passes selected parser variant and custom rules into parse requests", async () => {
    ...
    await user.selectOptions(
      view.getByRole("combobox", { name: /Parser variant/i }),
      "st",
    );
    await user.click(view.getByRole("button", { name: /Add custom rule/i }));
    await user.type(
      view.getByRole("textbox", { name: /Custom rule action 1/i }),
      "Warp",
    );
    await user.selectOptions(
      view.getByRole("combobox", { name: /Custom rule action type 1/i }),
      "jump",
    );
  ...
  it("renders parser warnings prominently after successful parsing", async () => {
    ...
    await waitFor(() => {
      expect(view.getByRole("region", { name: /Parser warnings/i }))
        .toBeInTheDocument();
    });
    ...
    expect(
      view.getByText(
        /Dynamic renpy\.call target cannot be resolved statically: dynamic_target/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("exports PNG via file-saver for each click", async () => {
    ...
    const exportBtn = view.getByRole("button", {
  ...
  it("exports SVG and JSON formats successfully", async () => {
    ...
    expect(exportedJson.nodes.some((n) => typeof n.role === "string")).toBe(
      true,
    );
    expect(exportedJson.edges.some((e) => typeof e.kind === "string")).toBe(
      true,
    );
  });
  ...
  it("exports a privacy-safe debug bundle by default and includes opt-in fields when enabled", async () => {
    ...
    await user.click(
      view.getByRole("button", { name: /Export debug bundle/i }),
    );
    ...
    await user.click(
      view.getByRole("checkbox", { name: /Include file names/i }),
    );
    await user.click(
      view.getByRole("checkbox", { name: /Include raw\/script details/i }),
    );
    await user.click(
      view.getByRole("checkbox", { name: /Include extra diagnostics/i }),
    );
    await user.click(
      view.getByRole("button", { name: /Export debug bundle/i }),
    );
    ...
    expect(Array.isArray(optedInBundle.graph.nodes[0]?.dialogueLines)).toBe(
      true,
    );
  ...
  it("shows debug export and opens a prefilled GitHub issue in the error panel", async () => {
    ...
    await waitFor(() => {
      expect(view.getByRole("button", { name: /Export Debug Bundle/i }))
        .toBeInTheDocument();
      expect(view.getByRole("button", { name: /Open new GitHub issue/i }))
        .toBeInTheDocument();
    });
    ...
    await user.click(
      view.getByRole("button", { name: /Open new GitHub issue/i }),
    );
    expect(openSpy).toHaveBeenCalledTimes(1);
    const issueUrl = openSpy.mock.calls[0]?.[0] as string;
    expect(issueUrl).toContain(
      "github.com/JavaGamer/RenpyWebFlowchartViewer/issues/new",
    );
  ...
  it("shows call-return edges only when the show call returns toggle is enabled", async () => {
    ...
    await user.click(
      view.getByRole("button", { name: /Show advanced controls/i }),
    );
  ...
  it("renders a skip link and persistent upload guidance text", () => {
    ...
    expect(view.getByRole("link", { name: /Skip to flowchart/i }))
      .toHaveAttribute("href", "#flowchart-main");
    expect(
      view.getByText(
        /Upload a Ren'Py project folder to visualize script structure, search dialogue, and export flowcharts/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("filters visible nodes via search and supports chapter collapse controls", async () => {
    ...
    const initialCount = parseInt(
      view.getByTestId("rf-node-count").textContent ?? "0",
      10,
    );
    ...
    const chapterToggle = screen.getByRole("button", {
      name: /Collapse chapter chapter1/i,
    });
  ...
  it("shows and applies edge-type visibility filters", async () => {
    ...
    await user.click(
      screen.getByRole("checkbox", { name: /Show jump edges/i }),
    );
  ...
  it("shows upload limits feedback for excessive file count and total size", async () => {
    ...
    const manyFiles = Array.from(
      { length: 301 },
      (_, i) => makeRpyFile(`f${i}.rpy`, 'label a:\n    "x"\n'),
    );
    await user.upload(input, manyFiles);
    await waitFor(() => {
      expect(view.getByText(/Too many \.rpy files selected/i))
        .toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(view.getByText(/exceeds the 25 MiB import limit/i))
        .toBeInTheDocument();
    });
  });
  ...
  it("supports minimum dialogue filter, layout switching, zoom controls, and label subgraph controls", async () => {
    ...
    const searchMode = screen.getByRole("combobox", {
      name: /Dialogue search mode/i,
    });
    expect(searchMode).toHaveValue("auto");
    const minDialogueInput = screen.getByRole("spinbutton", {
      name: /Minimum dialogue lines/i,
    });
    ...
    const layoutSelect = screen.getByRole("combobox", {
      name: /Auto layout direction/i,
    });
    await user.selectOptions(layoutSelect, "LR");
    expect(layoutSelect).toHaveValue("LR");

    const relayoutBtn = screen.getByRole("button", {
      name: /Re-run auto layout/i,
    });
    ...
    const beforeCollapse = parseInt(
      view.getByTestId("rf-node-count").textContent ?? "0",
      10,
    );
    const collapseLabel = screen.getByRole("button", {
      name: /Collapse label start/i,
    });
    ...
    const labelFilter = screen.getByRole("searchbox", {
      name: /Filter label subgraphs/i,
    });
    await user.clear(labelFilter);
    await user.type(labelFilter, "start");
    expect(screen.getByRole("button", { name: /Expand label start/i }))
      .toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /Collapse label end/i })).not
      .toBeInTheDocument();

    await user.click(
      screen.getByRole("button", {
        name: /Expand all visible label subgraphs/i,
      }),
    );
    await waitFor(() => {
      const count = parseInt(
    ...
    await user.click(
      screen.getByRole("button", {
        name: /Collapse all visible label subgraphs/i,
      }),
    );
    await waitFor(() => {
      const count = parseInt(
  ...
  it("resets label collapse state on each new import", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /Collapse label start/i }),
    );
    await waitFor(() => {
      const count = parseInt(
        view.getByTestId("rf-node-count").textContent ?? "0",
    ...
    await user.click(
      view.getByRole("button", { name: /Upload a different folder/i }),
    );
    ...
    expect(screen.getByRole("button", { name: /Collapse label start/i }))
      .toBeInTheDocument();
  });
  ...
  it("handles PNG and SVG export failures without crashing", async () => {
    ...
    await user.click(
      view.getByRole("button", { name: /Export flowchart as PNG/i }),
    );
    await waitFor(() => {
    ...
    await user.click(
      view.getByRole("button", { name: /Export flowchart as SVG/i }),
    );
    await waitFor(() => {
      expect(consoleError).toHaveBeenCalledWith(
        "SVG export failed:",
