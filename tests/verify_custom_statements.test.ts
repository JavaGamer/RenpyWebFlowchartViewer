...
test(
  "verify parsing of custom timedchoice, gameover, and title statements with generic mock data",
  async () => {
    const fileContent = `
...
label label_final_exit:
  ...
  `;
    ...
    const timeoutIncoming = result.edges.filter((e) =>
      e.target === "label_choice_timeout"
    );
    console.log(
      `label_choice_timeout incoming edge count: ${timeoutIncoming.length}`,
    );
    expect(timeoutIncoming.length).toBeGreaterThan(0);

    const timeoutEdge = timeoutIncoming.find((e) =>
      e.timeout?.isTimeout === true
    );
    expect(timeoutEdge).toBeDefined();
    expect(timeoutEdge?.timeout?.durationSeconds).toBe(3.0);
    console.log(
      "Successfully verified timedchoice timeout edge to label_choice_timeout!",
    );

    // 2. Verify incoming edges to label_final_exit
    const finalExitIncoming = result.edges.filter((e) =>
      e.target === "label_final_exit"
    );
    console.log(
      `label_final_exit incoming edge count: ${finalExitIncoming.length}`,
    );
    expect(finalExitIncoming.length).toBeGreaterThanOrEqual(1);

    const finalExitTimeoutEdge = finalExitIncoming.find((e) =>
      e.timeout?.isTimeout === true
    );
    expect(finalExitTimeoutEdge).toBeDefined();
    expect(finalExitTimeoutEdge?.timeout?.durationSeconds).toBe(1.1);
    console.log(
      "Successfully verified timedchoice timeout edge to label_final_exit!",
    );
    ...
    console.log(
      "Successfully verified no misparsed custom statements in dialogue!",
    );
  },
  15000,
);
