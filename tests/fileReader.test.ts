...
describe('readFileAsText', () => {
  ...
  it('resolves when FileReader returns text', async () => {
    ...
    await expect(readFileAsText(new File(['x'], 'ok.rpy'))).resolves.toBe('label start:');
  });
  ...
  it('rejects with FileReadError when FileReader result is not text', async () => {
    ...
    await expect(readFileAsText(new File(['x'], 'bad.rpy'))).rejects.toBeInstanceOf(FileReadError);
  });
  ...
  it('rejects with FileReadError when FileReader aborts', async () => {
    ...
    await expect(readFileAsText(new File(['x'], 'cancelled.rpy'))).rejects.toMatchObject({
      name: 'FileReadError',
    });
  });
});
