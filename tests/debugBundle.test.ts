  ...
  it("redacts file names, raw script details, and identifiers by default", () => {
    const bundle = buildDebugBundle({
      ...
      parseDiagnostics: [{
        ...
        message:
          "Dynamic renpy.call target cannot be resolved statically: secret_target",
      }],
      privacy: DEFAULT_DEBUG_BUNDLE_PRIVACY_OPTIONS,
    });

    const appMetadata = bundle.app as {
      errorMsg?: string;
      errorSummary?: string;
      parseProgress: { currentFile?: string } | null;
    };
    ...
    const firstEdge = bundle.graph.edges[0] as {
      id: string;
      source: string;
      target: string;
      label?: string;
  ...
  it("includes sensitive fields only when opted in", () => {
    const bundle = buildDebugBundle({
      ...
      parseDiagnostics: [{
        ...
        message:
          "Dynamic renpy.jump target cannot be resolved statically: target_var",
      }],
      privacy: {
    ...
    const firstWarning = (bundle.warnings?.[0] ?? {}) as {
      chapter?: string;
...
describe("debug bundle edge cases", () => {
  ...
  it("buildDebugBundle aliases parentLabelId in nodes when privacy is off", () => {
    ...
    const menuNode = bundle.graph.nodes.find((n) =>
      (n as { id: string }).id === "n2"
    ) as
