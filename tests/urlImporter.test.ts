...
import { resolveGithubUrl, fetchFilesFromUrl } from '../src/application/urlImporter';
...
describe('resolveGithubUrl', () => {
  it('resolves standard GitHub repo URL to main branch ZIP download', () => {
    expect(resolveGithubUrl('https://github.com/JavaGamer/RenpyWebFlowchartViewer')).toBe(
      'https://github.com/JavaGamer/RenpyWebFlowchartViewer/archive/refs/heads/main.zip'
    );
  });

  it('keeps raw script URL unmodified', () => {
    const rawUrl = 'https://raw.githubusercontent.com/JavaGamer/RenpyWebFlowchartViewer/main/src/App.tsx';
...
describe('fetchFilesFromUrl', () => {
  ...
  it('fetches and returns a raw .rpy script file', async () => {
    ...
    const result = await fetchFilesFromUrl('https://example.com/game/script.rpy');
    expect(fetchSpy).toHaveBeenCalledWith('https://example.com/game/script.rpy');
  ...
  it('fetches a .zip archive and calls extractRpyFilesFromZip', async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(mockResponse);
    const mockExtracted: UploadedFile[] = [{ name: 'extracted.rpy', size: 10, text: async () => 'extracted' }];
  ...
  it('throws a network/CORS error when fetch fails', async () => {
    ...
    await expect(fetchFilesFromUrl('https://example.com/cors-blocked.rpy')).rejects.toThrow(
  ...
  it('throws an error when response status is not ok', async () => {
    ...
    await expect(fetchFilesFromUrl('https://example.com/missing.rpy')).rejects.toThrow(
  ...
  it('throws an error when fetched URL points to an unsupported file type', async () => {
    ...
    await expect(fetchFilesFromUrl('https://example.com/pic.png')).rejects.toThrow(
