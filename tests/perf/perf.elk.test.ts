...
const outPath = path.join(perfDataDir, "elk-results.json");

function generateDataset(
  targetDir: string,
  files: number,
  labelsPerFile: number,
  menusEvery: number,
) {
...
function ensureBenchmarkDatasets() {
  ...
  const missing = requiredDatasets.some((name) =>
    !existsSync(path.join(generatedDataDir, name))
  );
  if (!missing) return;
    ...
    async () => {
      ...
      for (const dataset of datasets) {
        ...
        const visibleEdges = buildVisibleEdges({
          ...
          visibleNodeIds,
          edgeColor: "#4b5563",
          largeGraphMode: parsed.nodes.length > 180 ||
            parsed.edges.length > 320,
        });
        const renderTransformMs = performance.now() - renderStarted;

        const exportEstimateStarted = performance.now();
        const exportPayloadBytesEstimate =
          JSON.stringify({ nodes: visibleNodes, edges: visibleEdges }).length;
        ...
        results.push({
          ...
          parsePerFileAvgMs: Number(
            (
              fileDurations.reduce((sum, ms) => sum + ms, 0) /
              Math.max(fileDurations.length, 1)
            ).toFixed(2),
          ),
      ...
      expect(results.every((result) => Number(result.layoutMs) > 0)).toBe(true);
    },
    180_000,
  );
});
