...
describe("useParserRuleSettingsStore persistence", () => {
  ...
  it("returns defaults when storage is empty", () => {
    ...
    expect(state.selectedVariant).toBe(
  ...
  it("adds and removes custom rules for the active variant", () => {
    ...
    useParserRuleSettingsStore.getState().updateCustomRule(0, {
      actionName: "Warp",
      actionKind: "call",
    });
  ...
  it("addCustomRule appends a new empty rule for the currently selected variant", () => {
    ...
    expect(state.customRulesByVariant.st[0]).toEqual({
      actionName: "",
      actionKind: "jump",
    });
