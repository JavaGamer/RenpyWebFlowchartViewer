...
describe("validateRpyUpload", () => {
  it("returns no error for null file list", () => {
    expect(validateRpyUpload(null)).toEqual({
      rpyFiles: [],
      errorMessage: null,
    });
  });
  ...
  it("returns an error when no .rpy files are present", () => {
    ...
    expect(validateRpyUpload(files).errorMessage).toMatch(
      /No \.rpy files found/i,
    );
  });
  ...
  it("returns an error when a single .rpy file exceeds the per-file size limit", () => {
    const oversized = new File(
      [new Uint8Array(MAX_RPY_FILE_SIZE_BYTES + 1)],
      "huge.rpy",
    );
  ...
  it("accepts a .rpy file at exactly the per-file size limit", () => {
    const exact = new File(
      [new Uint8Array(MAX_RPY_FILE_SIZE_BYTES)],
      "big.rpy",
    );
