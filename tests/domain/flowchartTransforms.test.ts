...
describe("flowchartTransforms", () => {
  ...
  it("progressive layout handles more than PROGRESSIVE_LAYOUT_NODE_LIMIT nodes", () => {
    const manyNodes: FlowNode[] = Array.from({
      length: PROGRESSIVE_LAYOUT_NODE_LIMIT + 5,
    }, (_, i) => ({
      id: `n${i}`,
      type: "LABEL" as const,
      label: `n${i}`,
  ...
  it("progressive layout respects previousPositions for overflow nodes", () => {
    const manyNodes: FlowNode[] = Array.from({
      length: PROGRESSIVE_LAYOUT_NODE_LIMIT + 2,
    }, (_, i) => ({
      id: `n${i}`,
      type: "LABEL" as const,
      label: `n${i}`,
    ...
    const result = applyDagreLayout(manyNodes, [], "TB", {
      progressive: true,
      previousPositions: previousPos,
    });
    const overflowNode = result.nodes.find((n) =>
      n.id === `n${PROGRESSIVE_LAYOUT_NODE_LIMIT}`
    );
  ...
  it("buildVisibleNodes returns previous node instance when nothing has changed", () => {
    ...
    expect(second.find((n) => n.id === "start")).toBe(
      first.find((n) => n.id === "start"),
    );
  });
  ...
  it("evaluates conditional reachability and discovers mock flags from edge conditions", () => {
    ...
    expect(
      visibility.edgeConditionStateById.get("jump_decision_1__branch_true"),
    ).toBe("unreachable");
    expect(
      visibility.edgeConditionStateById.get("jump_decision_1__branch_false"),
    ).toBe("unknown");
  });
