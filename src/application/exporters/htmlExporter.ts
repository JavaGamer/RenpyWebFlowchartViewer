...
export function exportToHtmlBundle(svgDataUrl: string): string {
  const isSafeUrl =
    /^data:image\/(?:svg\+xml|png|jpeg|webp);/i.test(svgDataUrl) ||
    svgDataUrl.startsWith("blob:");
  const safeSrc = isSafeUrl ? escapeHtmlAttr(svgDataUrl) : "";
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    #container { width: 100vw; height: 100vh; display: flex; align-items: center; justify-content: center; user-select: none; }
    img { max-width: none; max-height: none; cursor: grab; user-select: none; -webkit-user-drag: none; }
...
<body>
  <div id="container">
    <img id="diagram" src="${safeSrc}" alt="Flowchart Diagram" draggable="false" />
  </div>
  <div id="controls">
