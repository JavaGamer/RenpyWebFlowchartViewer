...
export async function traverseFileSystemEntry(entry: FileSystemEntry, path = ''): Promise<UploadedFile[]> {
  const files: UploadedFile[] = [];

  const traverse = async (item: FileSystemEntry, currentPath: string): Promise<void> => {
    if (item.isFile) {
...
export async function traverseDataTransferItems(items: DataTransferItemList): Promise<UploadedFile[]> {
