  ...
  return (
    <div id="viewer-advanced-controls" className="flex flex-col gap-5" role="group" aria-label="Advanced controls">
      ...
      <div className="flex flex-col gap-2" role="group" aria-label="Layout and focus controls">
        <h3 className={cn("text-[11px] font-bold uppercase tracking-wider", isDark ? "text-slate-500" : "text-gray-400")}>Layout & Focus</h3>
        <div className={cn("grid grid-cols-3 gap-2 p-3 rounded-lg border", isDark ? "bg-slate-800/40 border-slate-700/60" : "bg-gray-50/50 border-gray-100")}>
          <label className={cn("text-xs flex flex-col gap-1 font-medium", isDark ? "text-slate-300" : "text-gray-700")}>
            <span className={cn("flex items-center gap-1", isDark ? "text-slate-400" : "text-gray-600")}>
          ...
          <label className={cn("text-xs flex flex-col gap-1 font-medium col-span-3", isDark ? "text-slate-300" : "text-gray-700")}>
            Focus label
          ...
          <div className="col-span-2 flex justify-between items-center pt-1">
            ...
            <span className={cn("text-[11px]", isDark ? "text-slate-400" : "text-gray-500")} aria-live="off">
              {!focusNodeId
      ...
      <div className="flex flex-col gap-2" role="group" aria-label="Advanced graph filters">
        <h3 className={cn("text-[11px] font-bold uppercase tracking-wider", isDark ? "text-slate-500" : "text-gray-400")}>Graph Filters</h3>
        <div className={cn("flex flex-col gap-3.5 p-3 rounded-lg border text-xs", isDark ? "bg-slate-800/40 border-slate-700/60" : "bg-gray-50/50 border-gray-100")}>
          <div className="grid grid-cols-2 gap-3">
            <label className="inline-flex items-center gap-2 cursor-pointer select-none">
              ...
              />
              <span className={cn("font-medium", isDark ? "text-slate-350" : "text-gray-700")}>Show call returns</span>
            </label>
            <label className="inline-flex items-center gap-2 cursor-pointer select-none">
              <input
                type="checkbox"
              ...
              />
              <span className={cn("font-medium", isDark ? "text-slate-350" : "text-gray-700")}>Show media cues</span>
            </label>
            <label className="inline-flex items-center gap-2 cursor-pointer select-none col-span-2">
              ...
              />
              <span className={cn("font-medium mr-1", isDark ? "text-slate-350" : "text-gray-700")}>Large graph mode</span>
          ...
          <div className={cn("text-[11px] border-t pt-2", isDark ? "text-slate-400 border-slate-750" : "text-gray-500 border-gray-105")} role="status" aria-live="polite">
            Status: {largeGraphModeStatusText}
          </div>

          <div className={cn("flex flex-col gap-2 border-t pt-2", isDark ? "border-slate-750" : "border-gray-105")}>
            <span className={cn("font-semibold text-xs", isDark ? "text-slate-300" : "text-gray-700")}>Visible Edge Kinds</span>
            <div className="grid grid-cols-2 gap-2">
              {(['sequence', 'jump', 'call', 'call_return'] as const).map((kind) => (
                <label key={kind} className="inline-flex items-center gap-2 cursor-pointer select-none">
                  ...
                  />
                  <span className={cn("text-[11px] capitalize", isDark ? "text-slate-400" : "text-gray-600")}>{kind.replace('_', ' ')}</span>
                </label>
              ))}
            </div>
          </div>
      ...
      {(chapters.length > 0 || labels.length > 0) && (
        <div className="flex flex-col gap-2" role="group" aria-label="Chapter and label subgraph filters">
          <h3 className={cn("text-[11px] font-bold uppercase tracking-wider", isDark ? "text-slate-500" : "text-gray-400")}>Subgraphs</h3>
          <div className={cn("flex flex-col gap-3.5 p-3 rounded-lg border text-xs", isDark ? "bg-slate-800/40 border-slate-700/60" : "bg-gray-50/50 border-gray-100")}>
            {chapters.length > 0 && (
              <div className="flex flex-col gap-2">
                <span className={cn("font-semibold text-xs", isDark ? "text-slate-350" : "text-gray-700")}>Chapter Subgraphs</span>
            ...
            {labels.length > 0 && (
              <div className={cn("flex flex-col gap-2.5 border-t pt-2.5", isDark ? "border-slate-750" : "border-gray-105")}>
                <div className="flex justify-between items-center">
                  <span className={cn("font-semibold text-xs", isDark ? "text-slate-300" : "text-gray-700")}>Label Subgraphs</span>
                  <span className={cn("text-[11px]", isDark ? "text-slate-450" : "text-gray-500")} aria-live="polite">
                ...
                <div className="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto pr-1">
                  {visibleSubgraphLabels.length === 0 ? (
                    <span className={cn("text-[11px]", isDark ? "text-slate-450" : "text-gray-500")}>No labels match the filter.</span>
                  ) : (
                    <>
      ...
      <div className="flex flex-col gap-2" role="group" aria-label="Mock state simulation controls">
        <h3 className={cn("text-[11px] font-bold uppercase tracking-wider", isDark ? "text-slate-500" : "text-gray-400")}>Conditional Simulation</h3>
        <div className={cn("flex flex-col gap-3.5 p-3 rounded-lg border text-xs", isDark ? "bg-slate-800/40 border-slate-700/60" : "bg-gray-50/50 border-gray-100")}>
          <div className="grid grid-cols-2 gap-3">
            <label className={cn("text-xs flex flex-col gap-1 font-medium", isDark ? "text-slate-355" : "text-gray-700")}>
              Unreachable paths
              <select
          ...
          <div className={cn("flex flex-col gap-2 border-t pt-2.5", isDark ? "border-slate-750" : "border-gray-105")}>
            <span className={cn("font-semibold text-xs", isDark ? "text-slate-300" : "text-gray-700")}>Condition Flags</span>
            <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
              {discoveredFlags.length === 0 ? (
                <span className={cn("text-[11px]", isDark ? "text-slate-450" : "text-gray-500")}>No condition flags discovered.</span>
              ) : (
                discoveredFlags.map((flag) => (
                  <label key={flag} className={cn("flex items-center justify-between gap-1.5 text-xs border rounded px-2 py-1", isDark ? "bg-slate-800 border-slate-700/60 text-slate-200" : "bg-white border-gray-100 text-gray-700")}>
                    <span className="truncate flex-1 font-medium" title={flag}>{flag}</span>
                    <select
