...
export async function tokenizeOneFile(
  ...
  const cacheKey = fileIndex !== undefined && options.fileCacheKeys?.[fileIndex]
  ...
  const { document, nodes: tokenTree } = await renpyParse(normalizedContent);
