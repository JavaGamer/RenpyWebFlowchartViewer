...
function emitShadowedTargetResolutionDiagnostics(state: ParseGraphState): void {
  ...
  for (const [labelName, definitionCount] of state.labelDefinitionCountByName.entries()) {
    if (definitionCount <= 1) continue;
    const canonicalId = state.canonicalLabelIdByName.get(labelName) ?? labelName;
  ...
  for (const edge of state.edges) {
    ...
    addParseDiagnostic(
      state,
      {
        ...
        message: `Edge "${edge.id}" resolves to canonical label "${edge.target}" while duplicate shadowed definitions exist for that label name.`,
        recoveryAction: 'Rename duplicate labels or keep one canonical definition to remove ambiguous target resolution.',
      },
